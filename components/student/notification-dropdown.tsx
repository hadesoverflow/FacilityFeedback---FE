    ...
    <DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DropdownMenuContent align="end" className="w-96">
        <div className="flex items-center justify-between p-3 border-b">
          ...
          {unreadCount > 0 && (
            <Button variant="ghost" size="sm" onClick={markAllAsRead} className="text-xs text-emerald-600">
            ...
            <div className="divide-y">
              {notifications.map((notif) => {
                ...
                return (
                  <div
                    key={notif.id}
                    className={`p-3 hover:bg-slate-50 transition-colors ${!notif.isRead ? "bg-emerald-50" : ""}`}
                  >
                    <div className="flex items-start gap-3">
                      <div className={`p-2 rounded-full ${style.bg}`}>
                        <Ticket className={`w-4 h-4 ${style.icon}`} />
                      </div>
